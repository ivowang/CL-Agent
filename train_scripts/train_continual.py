...
Usage:
    ...
    python train_scripts/train_continual.py --config-name continual_learning_9tasks
    
    # O-LoRA: orthogonal subspace learning - train from scratch
    python train_scripts/train_continual.py --config-name continual_learning_9tasks_olora
...
Or use the shell scripts:
    bash runs/run_baseline.sh  # Baseline
    bash runs/run_olora.sh     # O-LoRA
"""
...
_LOCAL_VERL_PATH = os.path.join(_PROJECT_ROOT, "verl")
...
from cl_methods.base import CLMethodConfig
from cl_methods.olora import OLoRAConfig, OLoRACLMethod
from cl_methods.sdlora import SDLoRAConfig, SDLoRACLMethod
from cl_methods.naive import NaiveCLConfig, NaiveCLMethod
from cl_methods.l2p import L2PConfig, L2PCLMethod
...
def run_continual_training(config, start_global_step, checkpoint_dir, timestamp,
    ...
    from trainer.cl_agent_trainer import ContinualLearningAgentTrainer
...
@hydra.main(version_base=None, config_path="../config", config_name="continual_learning_9tasks")
def main(config):
    """
